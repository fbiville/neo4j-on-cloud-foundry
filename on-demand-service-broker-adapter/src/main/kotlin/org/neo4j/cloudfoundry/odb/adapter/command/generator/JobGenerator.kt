...
class JobGenerator(private val passwordGenerator: PasswordGenerator) {
    fun generateJob(job: String, serviceDeployment: ServiceDeployment): Either<ManifestCommandError, ManifestJob> {
        ...
        return Either.Right(ManifestJob(job, release.name!!, properties = JobProperties(passwordGenerator.generate())))
    }
